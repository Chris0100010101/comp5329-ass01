...
#         attn = torch.bmm(q, k.transpose(1, 2)) * self.scale  # 修复：加上 scale
#         attn = mask_logits(attn, attn_mask)
#         attn = F.softmax(attn, dim=2)
#         attn = self.drop(attn)

#         out = torch.bmm(attn, v)  # [B*h, L, d_k]
#         out = out.view(self.num_heads, batch_size, length, self.d_k)  # 修复：reshape 顺序
...
#         for i, conv in enumerate(self.convs):
#             out = self.norms[i](res)  # 修复：pre-norm 在子层之前
#             out = conv(out)
#             out = self.act(out)
#             out = out + res
#             if (i + 1) % 2 == 0:
...
#         out = self.normb(res)  # 修复：attention 的 pre-norm
#         out = res + self.drop(self.self_att(out, mask))
#         res = out

#         out = self.norme(res)  # FFN pre-norm
#         out = self.fc(out.transpose(1, 2)).transpose(1, 2)
#         out = out + res
#         out = self.drop(out)
#         return out
...
def mask_logits(target: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
    if mask.dtype != torch.bool:
        mask = mask.bool()
    return target.masked_fill(mask, float('-inf'))
...
class PosEncoder(nn.Module):
    def __init__(self, d_model: int, length: int):
        ...
        pos = torch.arange(length, dtype=torch.float32).unsqueeze(0)
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, x: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
        ...
        if mask.dtype != torch.bool:
            mask = mask.bool()
        attn_mask = mask.unsqueeze(1).expand(-1, length, -1)           # [B, L, L]
        attn_mask = attn_mask.unsqueeze(0).expand(self.num_heads, -1, -1, -1)  # [h, B, L, L]
        attn_mask = attn_mask.reshape(self.num_heads * batch_size, length, length)  # [h*B, L, L]
        ...
        out = torch.bmm(attn, v)  # [h*B, L, d_k]
...
class EncoderBlock(nn.Module):
    def __init__(self, d_model: int, num_heads: int, dropout: float, conv_num: int, k: int, length: int,
                 init_name: str = "kaiming", act_name: str = "relu",
                 norm_name: str = "layer_norm", norm_groups: int = 8):
        super().__init__()
        self.convs = nn.ModuleList([DepthwiseSeparableConv(d_model, d_model, k, init_name=init_name) for _ in range(conv_num)])
        self.conv_drops = nn.ModuleList([Dropout(dropout * (i + 1) / conv_num) for i in range(conv_num)])
        self.drop = Dropout(dropout)
        ...
        self.conv_norms = nn.ModuleList([get_norm(norm_name, d_model, length, num_groups=norm_groups) for _ in range(conv_num)])
    ...
    def forward(self, x: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
        ...
        out = self.att_norm(res)
        out = res + self.drop(self.self_att(out, mask))
        res = out

        out = self.ffn_norm(res)
        out = self.fc(out.transpose(1, 2)).transpose(1, 2)
        out = self.act(out)
        out = out + res
        out = self.drop(out)
